...
from drift.auth.oculus import run_ticket_validation
...
class OculusCase(unittest.TestCase):
    ...
    def test_oculus(self):
        # Test success
        oculus_id = run_ticket_validation(user_id=123, access_token='success-token', nonce=self.nonce)
        self.assertTrue(oculus_id == 123)
