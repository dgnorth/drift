...
def get_admin_options(subparsers):
    ...
    p = subparsers.add_parser(
        'publish', 
        help='Publish a distribution of this project.',
        description='Builds a binary distribution, uploads and publishes to a PyPI server.'
        )
...
def use_command(args):
    ...
    # Due to a bug in a previous version of this function, keys were copied 
    # to the .drift folder. Let's clean up then:
    cleanups = [
        "deveast-key.pem",
        "dgn-dev-ec2.pem",
        "directive-pm.pem",
    ...
    for cleanit in cleanups:
        legacy_file = get_config_path(cleanit)
        if os.path.exists(legacy_file):
            print "Legacy action: Removing SSH key file", legacy_file
