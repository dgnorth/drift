# -*- coding: utf-8 -*-
"""
    Web Servers Factory for Flask applications.
    ---------------------------------------------

    Runs different flavours of web servers for a Flask application.
...
def run_app(app, web_server=None):
    ...
    """
    web_server = web_server or app.config.get("web_server", "flask")
    log.info("Using '%s' web server.", web_server)
    ...
    fn(app, app.config.get('PORT', 5000))
...
def run_twisted_server(app, port):
    ...
    sys.argv = ['runserver.py', 'web', '--port', str(port), '--wsgi', 'app.app']
